"""
Torch Fidelity doesn't support .npz reference statistics out of the box.
This wrapper adds that functionality by downloading the .npz file if a URL is provided
and loading the statistics from it, but remains the computation logic of Torch Fidelity intact.
...
def url_to_path(url_or_path: str):
    """
    Convert a URL to a local file path by downloading it if necessary.
    If the input is already a local path, it is returned as is.
    """
...
def calculate_metrics(**kwargs):
    """
    copied from torch_fidelity package and modified to support .npz reference.
